...
const VideoCallPage: React.FC = () => {
  ...
  const toggleFacilitatorVisibility = (checked: boolean) => {
    ...
    toast({
      ...
      description: checked ? "All participants can now see when you're in facilitator mode and any alerts." : "Facilitator mode is now only visible to you."
    });
  };
  const toggleAgenda = () => {
  ...
  return <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="absolute top-2 left-1/2 transform -translate-x-1/2 z-30 w-auto">
        {headerVisible ? <div className="bg-white/90 backdrop-blur-md px-3 py-1 rounded-full shadow-lg flex items-center gap-2 border border-gray-200/30">
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" className="text-gray-500 hover:text-gray-700 font-normal text-xs h-7 px-1.5 flex items-center gap-1" onClick={handleFacilitatorHint}>
                <Bell className="h-3.5 w-3.5" />
                Facilitator
              </Button>
              
              {/* New facilitator visibility option */}
              {facilitatorMode && <div className="flex items-center gap-1 text-xs text-gray-600">
                  <Checkbox id="facilitatorVisibility" checked={facilitatorVisibleToAll} onCheckedChange={toggleFacilitatorVisibility} className="h-3 w-3" />
            ...
            <div className="flex items-center gap-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-blue-500 hover:text-blue-700 font-normal text-xs h-7 px-1.5 flex items-center gap-1">
                ...
                <PopoverContent className="w-64 text-xs p-3">
                  <div className="flex flex-col gap-2">
                    <Button className="w-full justify-start bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100">
                      Answer
                    </Button>
                    <Button className="w-full justify-start bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-100">
                      Explain
              ...
              <Button variant="ghost" size="sm" onClick={toggleChat} className="text-gray-500 hover:text-gray-700 font-normal text-xs h-7 px-1.5 flex items-center gap-1">
                {chatVisible ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                {chatVisible ? "Hide Chat" : "Show Chat"}
              </Button>
              
              <Button variant="ghost" size="icon" onClick={toggleHeader} className="text-gray-500 hover:text-gray-700 h-6 w-6">
                <Settings className="h-3.5 w-3.5" />
              </Button>
              
              <Link to="/web-platform">
                <Button className="bg-blue-500 text-white hover:bg-blue-600 border-none px-2 py-1 rounded-full text-xs h-6">
            ...
            <Button variant="ghost" size="sm" onClick={toggleHeader} className="bg-white/90 backdrop-blur-md rounded-full shadow-sm border border-gray-200/30 px-1 py-0.5 h-5">
              <Settings className="h-3 w-3 text-gray-500" />
            </Button>
            
            <Link to="/web-platform">
              <Button variant="outline" size="sm" className="bg-blue-500 text-white hover:bg-blue-600 border-none px-1 py-0.5 h-5 rounded-full shadow-sm backdrop-blur-md text-xs flex items-center gap-1">
      ...
      {showNotification && headerVisible && <div className="absolute top-16 left-1/2 transform -translate-x-1/2 z-30 w-auto max-w-lg px-4 py-2">
      ...
      {showFacilitatorHint && <div className="absolute top-16 left-1/2 transform -translate-x-1/2 z-30 w-auto max-w-lg px-4 py-2">
          <Alert variant="default" className={`${facilitatorVisibleToAll ? 'bg-blue-500/90 text-white' : 'bg-white/90 text-gray-700'} backdrop-blur-sm border-blue-600/40 shadow-md`}>
      ...
      <div className="flex-grow relative">
        ...
        {chatVisible && <div className="absolute top-4 right-4 bottom-20 w-96 z-10 rounded-lg overflow-hidden shadow-xl">
